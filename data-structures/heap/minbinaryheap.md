...
Binary min-heap implemented using a resizing array.

![Heap represented as array](../../.gitbook/assets/image.png)
...
| Procedure | Time |
| :--- | :--- |
| `MinBinaryHeap(E[] keys)` | $$O(n),\ O(n)$$ |
| `void insert(E e)` | $$O(1),\ amortised\ O(log\ n)$$ |
| `E removeMin()` | $$O(log\ n),\ O(log\ n)$$ |
| `E remove(int i)` | $$O(log\ n),\ O(log\ n)$$ |
| `E getMin()` | $$O(1),\ O(1)$$ |
| `void increaseKey(int i, E e)` | $$O(log\ n),\ O(log\ n)$$ |
| `void decreaseKey(int i, E e)` | $$O(1),\ O(log\ n)$$ |
| `int size()` | $$O(1),\ O(1)$$ |
| `boolean isEmpty()` | $$O(1),\ O(1)$$ |
| `void merge(MinBinaryHeap<E> heap)` | $$O(n),\ O(n)$$ |

## Notes

1-based indexing to simplify computations.
